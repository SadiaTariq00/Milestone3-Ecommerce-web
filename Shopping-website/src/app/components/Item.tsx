...
const Menu = () => {
  // Directly define the Product interface here
  interface Product {
    id: number;
    name: string;
    price: number;
                    ...
                    {cart.map((product, index) => (
                      <li key={index} className="flex items-center justify-between mb-6">
                        <div className="flex items-center">
                          ...
                          />
                          <div className="ml-4">
                            <span className="text-lg font-medium text-slate-600 block">{product.name}</span>
                            {product.oldPrice && (
                              <span className="text-red-500 line-through mr-2">${product.oldPrice}</span>
                            )}
                            <span className="text-lg text-slate-900">${product.price}</span>
                            {product.rating && (
                              <p className="text-yellow-500 font-medium">⭐ {product.rating.toFixed(1)} / 5</p>
                            )}
                          </div>
                        </div>
                      </li>
        ...
        {isCheckout && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 text-black">
            <div className="bg-white p-8 rounded-lg shadow-lg w-full sm:w-96">
              ...
              <p className="text-lg text-red-700">Please confirm your order!</p>
              <div className="mt-4">
                <ul>
                  {cart.map((product, index) => (
                    <li key={index} className="flex justify-between mb-4">
              ...
              <div className="mt-6 flex justify-between">
                ...
                <button
                  onClick={() => alert("Are you sure? Your order is confirmed!")}
                  className="bg-green-700 text-white py-2 px-6 rounded-lg text-lg shadow-md hover:bg-green-900 transition duration-300"
                >
                  Confirm
                </button>
